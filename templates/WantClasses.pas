 ...
 * Copyright (c) 2026-2027 Dustin T Waling, United States.                  *
 * All rights reserved.                                                     *
 *                                                                          *
 * Based on Original: (c) 2001-2003 Juancarlo Anez, Caracas, Venezuela.     *
 *                                                                          *
...
uses
  ...
  TypInfo,
  INIFiles,
  Variants,  // Required for all modern Delphi versions (6+)
...
const
  ...
  SupportedPropertyTypes = [
     ...
     tkLString,
     tkWString,
     tkUString,  // Delphi 12+ UnicodeString
     tkClass];
...
type
  ...
  public
    ...
    property Name:       string   read FName       write SetName stored True;
...
//  FRootPathSet:   boolean;
  ...
  published
    ...
    property ForEachList: boolean read FForEachList write FForEachList;
  ...
  protected
    ...
    procedure TaskFailureFmt(const FmtMsg: string;
...
begin
  ...
  Method.Data  := Self;
  ChildName := StringReplace(ChildName, '-', '_', [rfReplaceAll]);
...
begin
  ...
  Result := PChar(Result); // so no nulls sneak in (truncates at first null)
end;
...
//begin
    ...
    FRootPath := Path;
    SetProperty('rootpath', Path, true);
//  FRootPathSet := True;
//end;
end;
...
begin
  ...
  begin
    Log(Format('Skipping disabled target "%s"', [Target.Name]), vlVerbose);
    EXIT;
  end;
...
procedure TTask.TaskFailureFmt(const FmtMsg: String;
  Params: array of const; Addr: Pointer);
begin
  TaskFailure(Format(FmtMsg, Params), Addr);
end;
